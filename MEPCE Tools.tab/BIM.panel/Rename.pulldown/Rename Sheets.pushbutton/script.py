...
_________________________________________________________________
How-to:
(Optional) -> Select sheets in the project browser
-> Click on the button
-> Select Sheets
...
sel_sheets = [el for el in sel_elem if issubclass(type(el), Sheet)]

# If none selected- prompt selectviews from pyrevit.forms.select_sheets()
if not sel_sheets:
...
t = Transaction(doc, 'py-Rename Sheets')
